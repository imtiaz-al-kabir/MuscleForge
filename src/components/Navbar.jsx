    ...
    <nav className="absolute top-1 left-0 right-0 z-[100] flex items-center justify-between px-4 md:px-10 xl:px-8 h-24 md:h-32 w-full max-w-7xl mx-auto pointer-events-auto">
      ...
      <div className="hidden xl:flex gap-5 max-w-3xl h-[60px] bg-[#050505] text-white rounded-[50px] items-center justify-between pl-6 md:pl-12 pr-2 md:pr-4 border-2 border-white shadow-2xl mx-8">
        ...
        <div className="flex items-center gap-6 md:gap-12 shrink-0">
          <Link to="/" className="text-base font-bold tracking-wide text-white hover:text-[#7ac231] transition-colors whitespace-nowrap group relative">
            <span className="relative z-10">Home</span>
            <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-[#7ac231] group-hover:w-full transition-all duration-300"></span>
          </Link>
          <a href="#" className="text-base font-medium text-gray-300 hover:text-[#7ac231] transition-colors whitespace-nowrap group relative">
            <span className="relative z-10">Our gym location</span>
            <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-[#7ac231] group-hover:w-full transition-all duration-300"></span>
          </a>
        </div>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 bg-[#050505]/95 z-[200] flex flex-col items-center justify-center gap-8 xl:hidden p-10">
          ...
          <Link to="/" onClick={() => setIsMenuOpen(false)} className="text-2xl font-bold text-white hover:text-[#7ac231] transition-colors">Home</Link>
          <a href="#" onClick={() => setIsMenuOpen(false)} className="text-2xl font-medium text-gray-300 hover:text-[#7ac231] transition-colors">Our gym location</a>
